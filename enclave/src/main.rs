...
use libc::{
    c_int, sigaddset, sigemptyset, sigprocmask, sigset_t, sigwaitinfo, SIGINT, SIGTERM, SIG_BLOCK,
};
use nitro_cli::{
    common::{
    ...
    enclave_proc_comm::{
        enclave_proc_command_send_all, enclave_proc_connect_to_single, enclave_proc_spawn,
...
fn healthy() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let describe_info = match enclave_process_handle_all_replies::<EnclaveDescribeInfo>(
        &mut replies,
        0,
        false,
        vec![0],
...
fn boot() -> (String, Option<Console>) {
    ...
    if !Path::new("/run/nitro_enclaves").is_dir() {
        create_dir_all("/run/nitro_enclaves").expect("Failed to create /run/nitro_enclaves");
    }
